  ...
  <el-form-item
    ...
    <template v-if="element.type == 'table'">
      <el-table
        ...
        tooltip-effect="dark"
        height="350px"
        @selection-change="(val)=>changeFun(element.model,val)"
        align="center"
        style="width: 95%">
        ...
        <el-table-column label="操作" v-if="element.options.showDelete">
          <template slot-scope="scope">
            <el-button
              @click.native.prevent="deleteClick(scope.$index,element.options.defaultValue)"
              type="text"
              size="small">
              删除
            </el-button>
        ...
        <el-table-column label="" v-if="!element.options.showDelete"></el-table-column>
      </el-table>
      <el-button type="text" @click='addClick(element)'>添加</el-button>
    </template>
      ...
      if (this.data.list[index].type === 'table') {
        cloneData = {
          ...
          options: {
            ...cloneData.options,
            options: cloneData.options.options.map(item => ({ ...item })),
            pagination: { ...cloneData.options.pagination }
          }
        }
      }
      if (this.data.list[index].type === 'button') {
        cloneData = {
          ...cloneData,
          options: {
            ...cloneData.options,
            methodBody: { ...cloneData.options.methodBody },
            pagination: { ...cloneData.options.pagination }
          }
        }
      }
