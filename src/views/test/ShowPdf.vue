...
export default {
  ...
  methods: {
    ...
    getMainContentWidth() {
      const div = this.$refs.mainContent
      ...
      PDFJS.getDocument(pdfUrl).then(pdf => {
        ...
        for (let i = 1; i <= totalPage; i++) {
          pdf.getPage(i).then((page) => {
            ...
            const viewportFirst = page.getViewport(1)
            // 计算 scale值，this.contentWidth 为 ref = mainContent 的元素的宽度
            const scale = this.contentWidth / viewportFirst.width
     ...
     */
    pageUp() {
      const totalPages = this.totals.length
      if (this.pageNo > 0) {
